...
static void draw_header(Canvas* canvas, const SurveyModel* m) {
    ...
    if(m->finished) {
        ...
        snprintf(buf, sizeof(buf), "SURVEY %lus",
                 (unsigned long)(m->summary.elapsed_ms / 1000u));
    ...
    if(m->finished) {
        ...
        snprintf(buf, sizeof(buf), "%lu:%02lu", (unsigned long)(left_s / 60u), (unsigned long)(left_s % 60u));
...
static void draw_running(Canvas* canvas, const SurveyModel* m) {
    ...
    for(int k = 0; k < 62; k++) {
        ...
        if(y < RUN_WAVE_BASE) canvas_draw_line(canvas, x, RUN_WAVE_BASE, x, y);
        else canvas_draw_dot(canvas, x, RUN_WAVE_BASE);
    }
}
...
static void draw_verdict(Canvas* canvas, const SurveyModel* m) {
    ...
    canvas_draw_str_aligned(
        ...
        AlignCenter,
        AlignBottom,
        survey_verdict_advice(m->verdict));
